 ...
 * this button in different states. See [DemoButtonDefaults.filledButtonColors].
 * @param contentPadding The spacing values to apply internally between the container and the
 * content. See [DemoButtonDefaults.buttonContentPadding].
 * @param content The button content.
 */
@Composable
fun DemoFilledButton(
    onClick: () -> Unit,
    modifier: Modifier = Modifier,
    enabled: Boolean = true,
    small: Boolean = false,
    colors: ButtonColors = DemoButtonDefaults.filledButtonColors(),
    contentPadding: PaddingValues = DemoButtonDefaults.buttonContentPadding(small = small),
    content: @Composable RowScope.() -> Unit
) {
    Button(
        onClick = onClick,
        modifier = if (small) {
            modifier.heightIn(min = DemoButtonDefaults.SmallButtonHeight)
        } else {
            modifier
        },
        enabled = enabled,
...
fun DemoFilledButton(
    ...
    DemoFilledButton(
        ...
        small = small,
        colors = colors,
        contentPadding = DemoButtonDefaults.buttonContentPadding(
            small = small,
 ...
 * @param colors [ButtonColors] that will be used to resolve the container and content color for
 * this button in different states. See [DemoButtonDefaults.outlinedButtonColors].
 * @param contentPadding The spacing values to apply internally between the container and the
 * content. See [DemoButtonDefaults.buttonContentPadding].
 * @param content The button content.
 */
...
fun DemoOutlinedButton(
    ...
    enabled: Boolean = true,
    small: Boolean = false,
    border: BorderStroke? = DemoButtonDefaults.outlinedButtonBorder(enabled = enabled),
    colors: ButtonColors = DemoButtonDefaults.outlinedButtonColors(),
    contentPadding: PaddingValues = DemoButtonDefaults.buttonContentPadding(small = small),
    content: @Composable RowScope.() -> Unit
) {
    OutlinedButton(
 ...
 * @param small Whether or not the size of the button should be small or regular.
 * @param colors [ButtonColors] that will be used to resolve the container and content color for
 * this button in different states. See [DemoButtonDefaults.textButtonColors].
 * @param contentPadding The spacing values to apply internally between the container and the
 * content. See [DemoButtonDefaults.buttonContentPadding].
 * @param content The button content.
 */
...
fun DemoTextButton(
    ...
    enabled: Boolean = true,
    small: Boolean = false,
    colors: ButtonColors = DemoButtonDefaults.textButtonColors(),
    contentPadding: PaddingValues = DemoButtonDefaults.buttonContentPadding(small = small),
    content: @Composable RowScope.() -> Unit
) {
    TextButton(
 ...
 */
@Composable
private fun RowScope.DemoButtonContent(
    text: @Composable () -> Unit,
    leadingIcon: @Composable (() -> Unit)?,
    trailingIcon: @Composable (() -> Unit)?
) {
    if (leadingIcon != null) {
        Box(Modifier.sizeIn(maxHeight = DemoButtonDefaults.ButtonIconSize)) {
            leadingIcon()
        }
    }
    Box(
        Modifier
            .padding(
                start = if (leadingIcon != null) {
                    DemoButtonDefaults.ButtonContentSpacing
                } else {
                    0.dp
                },
