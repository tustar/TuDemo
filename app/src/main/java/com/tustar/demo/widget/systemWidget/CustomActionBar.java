...
public class CustomActionBar extends RelativeLayout {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mLeftTextColor = ta.getColor(
                R.styleable.CustomActionBar_left_textColor, 0);
        mLeftBackground = ta.getDrawable(
                R.styleable.CustomActionBar_left_background);
        mLeftText = ta.getString(R.styleable.CustomActionBar_left_text);

        mRightTextColor = ta.getColor(
                R.styleable.CustomActionBar_right_textColor, 0);
        mRightBackground = ta.getDrawable(
                R.styleable.CustomActionBar_right_background);
        mRightText = ta.getString(R.styleable.CustomActionBar_right_text);

        mTitleTextSize = ta.getDimension(
                R.styleable.CustomActionBar_title_textSize, 10);
        mTitleTextColor = ta.getColor(
                R.styleable.CustomActionBar_title_textColor, 0);
        mTitle = ta.getString(R.styleable.CustomActionBar_title_text);
        ...
        int margin = DisplayUtils.dp2px(context, 4);
        // 为组件元素设置相应的布局元素
        mLeftParams = new LayoutParams(
                LayoutParams.WRAP_CONTENT,
                LayoutParams.MATCH_PARENT);
        mLeftParams.addRule(RelativeLayout.ALIGN_PARENT_LEFT, TRUE);
        mLeftParams.setMargins(margin, margin, margin, margin);
        // 添加到ViewGroup
        ...
        mRightParams.setMargins(margin, margin, margin, margin);
